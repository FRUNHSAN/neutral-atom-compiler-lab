...
| 桥 | Δ Fidelity | 敏感性 | 说明 |
|---|---|---|---|
| keep-vs-move | 0.000 | HIGH | fidelity 无差异，但紧 slot 下 slot violation 168→0 |
...
|---|---|
| hard_threshold (Eq.15) | 168 |
| AL soft (joint optimization) | 0 |

### 结论

1. **常规场景（slot 充足）**：AL 软决策的 fidelity 与硬阈值在噪声水平内无差异（Δ≈0.4%）。AL 不伤害保真度。
2. **紧 slot 场景**：硬阈值的 per-qubit 独立决策系统性高估可用 slot 容量，产生 168 次约束违反。AL 软决策通过联合优化消除所有违反（168→0）。
