...
**六桥对应**：BR-keep-vs-move。这是我的核心改进点。ZAP 的 Eq.15 是线性冲突函数（硬阈值），我的 AL 软决策替换为：连续权重 w∈[0,1] + 全局 slot 容量约束 → 联合优化。在 slot 紧约束时，硬阈值的独立决策导致系统性 over-commit（violation 170→0）。
...
Eq.15 硬阈值 → AL 联合优化。已完成：紧 slot 试验显示 violation 170→0。待做：在真实 benchmark 上跑更全面的对比。
